...
def point_in_ellipse(x, y, x0, y0, a, b, theta):
    ...
    if pointInEllipse(x, y, x0, y0, b, a, theta):
        truth_value = True
    return truth_value
...
def two_point_correlation(image, phases):
    ...
    ft1 = np.fft.fft(edge_1, axis=1)
    ft2 = np.fft.fft(edge_2, axis=1)
    # The two-point correlation function is the inverse fft of the product of both ffts
    return np.fft.ifft(ft1 * ft2, axis=1).real
...
def two_point_stats(image, stat, r, phase, n=100, num_ellipses=600, plotting=False):
    '''
...
'''
# load in the image
image = io.imread("Images_From_Papers/actual_microstructure_3.jpg", as_gray=True)
...
image = io.imread("Images_From_Papers/actual_microstructure_3.jpg", as_gray=True)
probability = two_point_stats(image, lineal_path_func, 100, 'fibers', plotting=True)
plt.show()
