...
def varying_r(image, stat, csv, num, min_r, max_r, plotting=False, iterations=100):
    ...
    rs = np.linspace(min_r, max_r, num)
    for i in range(num):
        tot = 0
...
def find_ellipses_in_image(image):
    '''
...
image = io.imread("Manual_Microstructures/image0.png")
fiber_csv, void_csv = find_ellipses_in_image(image)
